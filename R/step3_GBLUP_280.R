...
pt <- proc.time()
results <- parallel::mclapply(51:80,function(pop.num){
...
proc.time()-pt
the.name <- paste("GBLUP.nodom.64.OP.pop_21_50", sep="")
assign(x=the.name,value=results)
save(list=the.name,file=paste("./data/results2/64/GBLUP.nodom.64/",the.name,".RData",sep=""))
