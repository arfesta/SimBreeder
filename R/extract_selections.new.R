extract_selections <- function(map.info, cross.design, past.tgv, 
  ...
  if (among.family.selection == "GBLUP") {
    ...
    prog.1 <- as.matrix( prog.markers[, , 1])
    colnames(prog.1) <- names(prog.phenos)
    prog.2 <- as.matrix(prog.markers[, , 2])
    colnames(prog.2) <- names(prog.phenos)
    ...
    genetic.relationships <- mclapply(1:length(prog.list),function(each.prog){
      if(each.prog == length(prog.list)){
        ...
        x <- first.prog[,1] == prog.markers[,each.prog:ncol(prog.markers),1]
        y <- first.prog[,1] == prog.markers[,each.prog:ncol(prog.markers),2]
        w <- first.prog[,2] == prog.markers[,each.prog:ncol(prog.markers),1]
        z <- first.prog[,2] == prog.markers[,each.prog:ncol(prog.markers),2]
        
        ll <- data.frame(rbind(x,y,w,z))
        ll[] <- lapply(ll,as.numeric)
        ll <- colSums(ll)
        all.match <- sapply(each.prog:ncol(prog.markers),function(x) { length(which(prog.markers[,x,1] == prog.markers[,x,2] & prog.markers[,x,1] == first.prog[,1] & first.prog[,1] == first.prog[,2]))})*2
        nd.p.h2 <- sapply(each.prog:ncol(prog.markers),function(x) { length(which(prog.markers[,x,1] == prog.markers[,x,2] & prog.markers[,x,1] == first.prog[,1] & prog.markers[,x,1] != first.prog[,2] | 
                                                                                    prog.markers[,x,1] == prog.markers[,x,2] & prog.markers[,x,1] == first.prog[,2] & prog.markers[,x,1] != first.prog[,1]))})
        rd.p.h2 <- sapply(each.prog:ncol(prog.markers),function(x) { length(which(first.prog[,1] == first.prog[,2] & first.prog[,1] == prog.markers[,x,1] & first.prog[,1] != prog.markers[,x,2] | 
                                                                                    first.prog[,1] == first.prog[,2] & first.prog[,1] == prog.markers[,x,2] & first.prog[,1] != prog.markers[,x,1]))})
        
        ll <- ll - all.match - nd.p.h2 - rd.p.h2
        ll/240
      }},mc.cores=num.cores)
    proc.time()-pt
    #elapsed 4643
    ...
    #t.gmat2 <- t(g.mat)
    g.mat[upper.tri(g.mat, diag = F)] <-  t(g.mat)[upper.tri( t(g.mat), diag = F)]
    proc.time() - pt
    rm(genetic.relationships); gc()
    A <- as_data_frame(as.matrix(getA(ped)))
    A = A[match(names(prog.phenos), colnames(A)), match(names(prog.phenos),  colnames(A))]
    the.data <- as.matrix(g.mat) * 0.99 + as.matrix(A) * 0.01
    rm(g.mat,A); gc()
    
    the.data <- solve(the.data)
    #the.data[lower.tri(the.data)] <- 0
    #the.data = solve(the.data)
    n.col <- ncol(the.data)
    h.2 <- var(prog.genetic.values)/var(prog.phenos)
    lambda <- (1 - h.2)/h.2
    I <- diag(n.col)
    sol <- solve(rbind(cbind(n.col, t(rep(1, n.col))), cbind(rep(1, n.col), (I + (lambda * the.data)))), 
                 matrix(c(sum(prog.phenos),  c(as.vector(prog.phenos)))))
    rm(I,the.data,prog.list); gc()
    sol <- sol[-1, 1]
  ...
  if (generation == 1) {
    ...
    colnames(new.parent.genos) <- select.ped.ids
    all.markers1 <- cbind(parent.info$all.markers[, , 1], 
                          new.parent.genos[marker.loci, , 1])
    all.markers2 <- cbind(parent.info$all.markers[, , 2], 
  ...
  if (relationship.matrix.type == "markers") {
    ...
    prog.list <- lapply(1:ncol(allele1), function(x) { cbind(allele1[, x], allele2[, x]) })
    genetic.relationships <- mclapply(1:length(prog.list),function(each.prog){
      if(each.prog == length(prog.list)){
        1
      } else {
        ...
        x <- first.prog[,1] == allele1[,each.prog:ncol(allele1)]
        y <- first.prog[,1] == allele2[,each.prog:ncol(allele1)]
        w <- first.prog[,2] == allele1[,each.prog:ncol(allele1)]
        z <- first.prog[,2] == allele2[,each.prog:ncol(allele1)]
        
        ll <- data.frame(rbind(x,y,w,z))
        ll[] <- lapply(ll,as.numeric)
        ll <- colSums(ll)
        
        all.match <- sapply(each.prog:ncol(all.alleles),function(x) { length(which(all.alleles[,x,1] == all.alleles[,x,2] & all.alleles[,x,1] == first.prog[,1] & first.prog[,1] == first.prog[,2]))})*2
        nd.p.h2 <- sapply(each.prog:ncol(all.alleles),function(x) { length(which(all.alleles[,x,1] == all.alleles[,x,2] & all.alleles[,x,1] == first.prog[,1] & all.alleles[,x,1] != first.prog[,2] | 
                                                                                   all.alleles[,x,1] == all.alleles[,x,2] & all.alleles[,x,1] == first.prog[,2] & all.alleles[,x,1] != first.prog[,1]))})
        rd.p.h2 <- sapply(each.prog:ncol(all.alleles),function(x) { length(which(first.prog[,1] == first.prog[,2] & first.prog[,1] == all.alleles[,x,1] & first.prog[,1] != all.alleles[,x,2] | 
                                                                                   first.prog[,1] == first.prog[,2] & first.prog[,1] == all.alleles[,x,2] & first.prog[,1] != all.alleles[,x,1]))})
