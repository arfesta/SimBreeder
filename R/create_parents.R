...
create_parents <- function(map.info, num.parents, max.delt.allele, heterozygous.markers = F, inbred.parents=F,
                           QTL.sd = 0.25, save = F){

  total.loci <- map.info$total.loci.num  # Specifies the total # of loci by pulling from the map object
  locus.names <- map.info$genetic.map$loci  # Specifies locus names from map object
  all.MAFs <- map.info$genetic.map$MAF     # Assign all.MAFs as the minor allele frequencies in the map object
  total.QTL <- map.info$total.QTL.num    # Total number of qtl pulled from map object
  QTLSNPs <- map.info$QTLSNP.loci      # A vector of the loci which are snpqtl
  num.SNPQTL <- map.info$total.SNPQTL.num  # the number of snpqtl pulled from map object
  num.QTL <- total.QTL - num.SNPQTL  # the number of additive qtl (rQTL)
  QTLoci <- sort(map.info$rQTL.loci)    # A vector of the loci which are additive QTL
  all.markers <- as.numeric(rownames(map.info$genetic.map)[-c(QTLSNPs,QTLoci)])
  ...
  while(anyNA(parents)){
    ...
    for(par in 1:num.parents){
      ...
      if(inbred.parents){
        allele1 <- d.marker.genotypes[par]
        allele2 <- r.marker.genotypes[par]
      } else {
  ...
  if(save){
    parfilename=paste("parent-SNPQTL-genotypes.txt")
    write.table(parent.SNP.genos,parfilename, quote = F, row.names = T, col.names = T, sep= "\t")

    parfilename=paste("parent-marker-genotypes.txt")
    write.table(parent.markers,parfilename, quote = F, row.names = T, col.names = T, sep= "\t")
  }

  parentinfo<-list(genos.3d=parents,delt.allele=result.parent,num.parents, parent.SNPQTL.matrix=parent.SNP.genos, parent.Marker.matrix=parent.markers, parent.IDs=par.IDs)
